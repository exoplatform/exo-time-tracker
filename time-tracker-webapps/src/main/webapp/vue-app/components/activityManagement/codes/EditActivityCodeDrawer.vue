<template>
  <exo-drawer
    ref="editActivityCodeDrawer"
    right
    attached
    class="">
    <template slot="title">
      {{ $t("exo.timeTracker.codes.editActivityCodeDrawer.toolbarTitle") }}
    </template>
...
export default {
  ...
  methods: {
    save() {
      ...
      this.$refs.editActivityCodeDrawer.close();
    },
    cancel() {
      this.activityCode = this.defaultItem;
      this.$refs.editActivityCodeDrawer.close();
    ...
    open(activityCode) {
      ...
      this.$refs.editActivityCodeDrawer.open();
    },
  }
};
</script>
